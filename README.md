...
> 실행 전 가상 환경이 활성화되어 있어야 합니다: `source agent/bin/activate`
...
> 가상 환경 활성화 필요: `source agent/bin/activate`
...
```bash
python -m pytest mock_server/tests/ -v
```

전체 테스트 (통합):

```bash
python -m pytest tests/ mock_server/tests/ -v
```
