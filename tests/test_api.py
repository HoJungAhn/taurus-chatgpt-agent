...
def test_cached_response():
    ...
    """
    with patch.object(similarity_service, "find_similar", return_value="캐시된 결과"):
...
def test_chatgpt_success():
    ...
    find_similar가 None을 반환(캐시 미스)하고,
    call_chatgpt가 성공적으로 결과를 반환하는 상황을 시뮬레이션한다.
    save()가 정확히 한 번 호출되어야 한다.
    """
    with patch.object(similarity_service, "find_similar", return_value=None), \
